...
-- Políticas para tiendas (solo admin)
DROP POLICY IF EXISTS "Admins pueden gestionar tiendas" ON public.tiendas;
...
-- Crear tabla relación cliente_tiendas
...
CREATE INDEX IF NOT EXISTS idx_cliente_tiendas_cliente ON public.cliente_tiendas(cliente_id);
CREATE INDEX IF NOT EXISTS idx_cliente_tiendas_tienda ON public.cliente_tiendas(tienda_id);

-- Habilitar RLS en cliente_tiendas
ALTER TABLE public.cliente_tiendas ENABLE ROW LEVEL SECURITY;

-- Políticas para cliente_tiendas (solo admin)
DROP POLICY IF EXISTS "Admins pueden gestionar relaciones cliente-tienda" ON public.cliente_tiendas;
...
-- Datos de ejemplo de tiendas (idempotente)
INSERT INTO public.tiendas (nombre, rut, direccion, telefono, email)
VALUES
